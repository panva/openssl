...
EVP_MD-TURBOSHAKE, EVP_MD-KANGAROOTWELVE
- The SHAKE / KECCAK family EVP_MD implementations

=head1 DESCRIPTION

Support for computing SHAKE, CSHAKE, CSHAKE-KECCAK, TurboSHAKE, or
KangarooTwelve (KT) digests through the B<EVP_MD> API.
...
TurboSHAKE is an Extendable Output Function (XOF) defined by RFC 9861. It
uses the same rate and capacity as SHAKE, but uses Keccak-p[1600,12] and a
domain separation byte.

KT-128 and KT-256 are KangarooTwelve Extendable Output Functions (XOFs)
defined by RFC 9861. They use tree hashing over TurboSHAKE and support a
customisation string.

=head2 Identities

The SHAKE, CSHAKE, and CSHAKE-KECCAK implementations are available in the FIPS
provider as well as the default provider. TurboSHAKE and KangarooTwelve are
available in the default provider. The following varieties are included:
...
Known names are "TURBOSHAKE-128" and "TURBOSHAKE128".

=item TURBOSHAKE-256

Known names are "TURBOSHAKE-256" and "TURBOSHAKE256".

=item KT-128

Known names are "KT-128" and "KT128".

=item KT-256

Known names are "KT-256" and "KT256".
...
The TURBOSHAKE-128 and KT-128 implementations have a default digest length of
32 bytes. The TURBOSHAKE-256 and KT-256 implementations have a default digest
length of 64 bytes.
...
variable length output. If it is not set, CSHAKE, TurboSHAKE, and
KangarooTwelve will use the default value. It is recommended to either use
...
Sets a customisation string used by CSHAKE or KangarooTwelve.
For CSHAKE and KangarooTwelve, it may be set as either a UTF8 string or an
octet string with a length of at most 512 bytes, and is empty by default.

=item "domain-separator" (B<OSSL_DIGEST_PARAM_DOMAIN_SEPARATOR>) <unsigned integer>

Sets or gets the TurboSHAKE domain separation byte. Valid values are 1 to 127,
inclusive. The default value is 31 (0x1f). This parameter is only supported by
TURBOSHAKE-128 and TURBOSHAKE-256.
...
For SHAKE-128, CSHAKE-128, TURBOSHAKE-128, and KT-128, to ensure the maximum
security strength of 128 bits, the output length passed to EVP_DigestFinalXOF()
should be at least 32.

For SHAKE-256, CSHAKE-256, TURBOSHAKE-256, and KT-256, to ensure the maximum
security strength of 256 bits, the output length passed to EVP_DigestFinalXOF()
should be at least 64.

KangarooTwelve uses TurboSHAKE domain separation bytes 0x06, 0x07, and 0x0b
internally. Protocols that use both KangarooTwelve and TurboSHAKE should avoid
these TurboSHAKE domain separator values.
...
=item RFC 9861 TurboSHAKE and KangarooTwelve
...
TURBOSHAKE-128, TURBOSHAKE-256, KT-128, and KT-256 were added in OpenSSL 4.1.
