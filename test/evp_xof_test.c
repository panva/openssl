...
static int kt_custom_null_utf8_test(void)
{
    static const unsigned char msg[] = { 0xff };
...
static int cshake_custom_octet_test_libctx(OSSL_LIB_CTX *libctx,
    const char *propq)
{
    static char function_name[] = "KMAC";
    static const unsigned char custom_ascii[] = "Custom";
    static const unsigned char custom_binary[] = {
        'C', 'u', 's', 't', 'o', 'm', 0x00, 'X', 0xff
    };
    unsigned char ascii_out[sizeof(cshake256_output)];
    unsigned char binary_out[sizeof(cshake256_output)];
    OSSL_PARAM params[3];

    params[0] = OSSL_PARAM_construct_utf8_string(
        OSSL_DIGEST_PARAM_FUNCTION_NAME, function_name, 0);
    params[1] = OSSL_PARAM_construct_octet_string(
        OSSL_DIGEST_PARAM_CUSTOMIZATION, (void *)custom_ascii,
        sizeof(custom_ascii) - 1);
    params[2] = OSSL_PARAM_construct_end();

    if (!digest_xof_with_params_ex(libctx, propq, "CSHAKE256",
    ...
    params[1] = OSSL_PARAM_construct_octet_string(
        OSSL_DIGEST_PARAM_CUSTOMIZATION, (void *)custom_binary,
        sizeof(custom_binary));

    return digest_xof_with_params_ex(libctx, propq, "CSHAKE256",
            shake256_input, sizeof(shake256_input), params, binary_out,
            sizeof(binary_out))
        && TEST_mem_ne(binary_out, sizeof(binary_out), cshake256_output,
            sizeof(cshake256_output));
}
...
static int cshake_custom_octet_test(void)
{
    return cshake_custom_octet_test_libctx(NULL, "provider=default")
        && cshake_custom_limit_test_libctx(NULL, "provider=default");
}

static int cshake_custom_octet_fips_test(void)
{
    ...
    ret = cshake_custom_octet_test_libctx(libctx, "fips=yes")
...
int setup_tests(void)
{
    if (test_get_argument_count() > 0) {
        if (test_get_argument_count() != 2
            ...
            return 0;
        ADD_TEST(cshake_custom_octet_fips_test);
        return 1;
    }
    ...
    ADD_TEST(kt_custom_null_utf8_test);
    ADD_TEST(kt_custom_limit_test);
    ADD_TEST(cshake_custom_octet_test);
